...
object FirExplicitApiDeclarationChecker : FirDeclarationSyntaxChecker<FirDeclaration, KtDeclaration>() {
    ...
    private fun explicitVisibilityIsNotRequired(declaration: FirMemberDeclaration, context: CheckerContext): Boolean {
        return when (declaration) {
            ...
            is FirCallableDeclaration -> {
                ...
                if (declaration is FirProperty && containingClass != null) {
                    // 2, 5
                    if (containingClass.isData || containingClass.classKind == ClassKind.ANNOTATION_CLASS) return true
                }
                // 3
    ...
    private fun checkExplicitReturnType(
        ...
        if (!returnTypeCheckIsApplicable(declaration, source, context)) return
    ...
    private fun returnTypeCheckIsApplicable(
        ...
        if (declaration.returnTypeRef.source?.kind is FirRealSourceElementKind) return false
        if (context.containingDeclarations.first().source?.elementType in codeFragmentTypes) return false
        ...
        return when (source.elementType) {
            ...
            KtNodeTypes.FUN -> source.getChild(KtTokens.EQ) != null
